      ...
      <Dialog open={!!modal} onOpenChange={() => closeModal()}>
        <DialogContent className="ios-sheet max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-2 mt-2">
            {modal?.type === 'recebimentos' && (<><p className="text-sm font-bold text-green-600 mb-2">Total: R$ {formatMoney(data.totalReceived || 0)}</p>{data.receivedInstallments?.map((inst: any) => (<div key={inst.id} className="bg-green-50 p-3 rounded-xl"><div className="flex justify-between"><div><p className="text-sm font-medium">{inst.client_name}</p><p className="text-xs text-gray-500">Parcela {inst.installment_number}x - {new Date(inst.payment_date).toLocaleDateString('pt-BR')}</p></div><p className="font-bold text-green-700">R$ {formatMoney(inst.paid_amount || inst.amount || 0)}</p></div></div>))}{data.extraPayments?.map((ep: any) => (<div key={ep.id} className="bg-green-50 p-3 rounded-xl border border-green-200"><div className="flex justify-between"><div><p className="text-sm font-medium">{ep.clients?.name}</p><p className="text-xs text-gray-500">Avulso - {new Date(ep.created_at).toLocaleDateString('pt-BR')}</p></div><p className="font-bold text-green-700">R$ {formatMoney(ep.amount || 0)}</p></div></div>))}{!data.receivedInstallments?.length && !data.extraPayments?.length && <p className="text-sm text-gray-400 text-center py-4">Nenhum recebimento</p>}</>)}
